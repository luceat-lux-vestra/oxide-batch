#!/usr/bin/env bash
# Fail closed when the dedicated M5 upgrade workflow drifts from its contract.

set -euo pipefail

workflow_path="${1:?usage: verify-ci-contract.sh <workflow-path>}"
contract="tests/fixtures/upgrade/execution-contract.json"
script_path="$(jq -er '.script_path' "${contract}")"
scope="tests/fixtures/upgrade/campaign-scope.json"
runner_source="crates/oxide-batch/tests/upgrade/mod.rs"

fail() {
  echo "upgrade execution contract drift: $*" >&2
  exit 1
}
...
# matrix, a changed command. Exact git blob identity catches all of that, and
...
# The historical revision is a runtime input the workflow itself never
# mentions — it is resolved by the runner from inside the checkout the
# full-history contract above provides — but the contract, the scope
# document, and the pinning constant in the runner's shared support module
# must all name the same commit, or "the revision the workflow can build" and
# "the revision the campaign says it built" are three different claims.
contract_revision="$(jq -er '.historical_schema2_runtime.revision' "${contract}")"
scope_revision="$(jq -er '.schema2_runtime.revision' "${scope}")"
test "${contract_revision}" = "${scope_revision}" ||
  fail "execution-contract.json names schema-2 runtime revision ${contract_revision} and ${scope} names ${scope_revision}"
grep -Fq -- "\"${contract_revision}\"" "${runner_source}" ||
  fail "${runner_source} does not pin SCHEMA2_RUNTIME_REVISION to ${contract_revision}, which the contract and the scope document both declare"
...
echo "upgrade execution contract matches ${workflow_path}"
