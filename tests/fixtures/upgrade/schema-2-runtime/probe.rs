//! Asks a schema-2 OxideBatch runtime to open a schema-3 database.
//!
...
//! runtime supported schema 2, copies this program into it as an example, and
//! runs it against a database the current migrator has upgraded to schema 3.
//! It is written here and compiled there because the runtime under test cannot
//! be built from the working tree — the supported schema version is a constant
//! of the crate, and a runtime that reports `2` is the one that shipped against
//! schema 2, not a reconstruction of it.
//!
...
//! database at schema 3, and what its migrator did with one.
//!
//! The migrator is asked as well as the runtime because an operator rolling a
//! deployment back runs both. A schema-2 migrator that treated a schema-3
...
const MARKER: &str = "M5_SCHEMA2_PROBE";

/// The variable naming the schema-3 database to attempt.
