...
database="oxide_batch_upgrade7"
restore_database="oxide_batch_upgrade7_restore"
...
upgrade7_psql <"${fixture_root}/design-gate/schema4-seed.sql"
...
upgrade7_psql <"${fixture_root}/design-gate/verify-schema5-upgrade.sql"
...
upgrade7_psql <"${fixture_root}/design-gate/verify-schema6-upgrade.sql"
...
echo "schema 6 to 7 migration took $(( $(date +%s) - upgrade_started ))s"

upgrade7_psql <"${fixture_root}/design-gate/verify-schema7-upgrade.sql"
...
if [[ ${reapply_status} -eq 0 ]]; then
  echo "schema 7 migration was applied twice" >&2
  exit 1
fi
if [[ "${reapply_output}" != *"schema version 6 is required before schema 7"* ]]; then
  echo "${reapply_output}" >&2
  echo "reapplying schema 7 returned an unexpected diagnostic" >&2
  exit 1
fi
...
docker exec \
  ...
  --format=custom \
  --schema=oxide_batch \
  >"${temporary_root}/schema7-populated.dump"
...
docker exec -i \
  ...
  --set ON_ERROR_STOP=on \
  --quiet \
  <"${fixture_root}/design-gate/verify-schema7-restore.sql"

echo "populated schema 6 to 7 upgrade and restore fixture passed"
