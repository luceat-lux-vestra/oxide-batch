...
# Schema-5 to schema-6 evidence starts from the populated schema-5 shape,
# applies only the scoped-resolution provenance migration, proves that the
# migration invents no prior provenance, and dump/restores representative
# value-free source-family records.
bash "${fixture_root}/design-gate/run-schema6-upgrade-gate.sh" \
  "${container_name}" \
  "${repository_root}" \
  "${fixture_root}" \
  "${temporary_root}"
