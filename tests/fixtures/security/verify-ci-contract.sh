#!/usr/bin/env bash
# Fail closed when the dedicated M5 security workflow drifts from its
# contract.

set -euo pipefail

workflow_path="${1:?usage: verify-ci-contract.sh <workflow-path>}"
contract="tests/fixtures/security/execution-contract.json"
script_path="$(jq -er '.script_path' "${contract}")"

fail() {
  echo "security execution contract drift: $*" >&2
  exit 1
}
...
require_absent() {
  local label="$1"
  local literal="$2"
  # Comment lines are excluded: the workflow explains the no-services
  # invariant in a comment that names the very key being forbidden, and that
  # explanation is not the drift this check exists to catch.
  grep -v -E '^\s*#' "${workflow_path}" | grep -Fq -- "${literal}" &&
    fail "${label} must not appear: ${literal}" || true
}

# The fail-closed authority. A literal check only proves an expected string is
# present; it says nothing about whether something *else* was added beside it
# — an extra trigger, a job-level permission override, a widened matrix, a
# changed command, a `services:` block. Exact git blob identity catches all of
# that, and any other byte-level change, uniformly: the file the workflow run
# actually executed either has the identity this contract recorded, or the
# contract is stale and the campaign has to be re-run and re-retained against
...
# The no-services invariant named in the contract. A `services:` block that
# started a plain PostgreSQL container would let the campaign happen to pass
# without ever exercising a certificate this run generated, which is not a
# convenience: it is the fixture the TLS claims depend on, silently removed.
if [ "$(jq -er '.no_services_block' "${contract}")" = "true" ]; then
  require_absent "services block" "services:"
fi
...
echo "security execution contract matches ${workflow_path}"
