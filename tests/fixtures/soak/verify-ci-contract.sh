#!/usr/bin/env bash
# Fail closed when the dedicated M5 soak workflow drifts from its contract.

set -euo pipefail

workflow_path="${1:?usage: verify-ci-contract.sh <workflow-path>}"
contract="tests/fixtures/soak/execution-contract.json"
script_path="$(jq -er '.script_path' "${contract}")"

fail() {
  echo "soak execution contract drift: $*" >&2
  exit 1
}
...
test "$(jq -er '.cargo_profile' "${contract}")" = "debug" ||
  fail "unexpected cargo profile in the soak contract"
! grep -Fq -- "--release" "${script_path}" || fail "debug campaign unexpectedly uses --release"

echo "soak execution contract matches ${workflow_path}"
