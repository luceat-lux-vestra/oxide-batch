#!/usr/bin/env bash
# Fail closed when the dedicated M6 Gate B workflow drifts from its
# contract.

set -euo pipefail

workflow_path="${1:?usage: verify-ci-contract.sh <workflow-path>}"
contract="tests/fixtures/gate-b/execution-contract.json"
script_path="$(jq -er '.script_path' "${contract}")"

fail() {
  echo "gate-b execution contract drift: $*" >&2
  exit 1
}
...
# beside it. Exact git blob identity catches all of that, and any other
# byte-level change, uniformly.
...
echo "gate-b execution contract matches ${workflow_path}"
