#!/usr/bin/env bash
# Fail closed when the dedicated M5 crash-restore workflow drifts from its
# contract.

set -euo pipefail

workflow_path="${1:?usage: verify-ci-contract.sh <workflow-path>}"
contract="tests/fixtures/crash-restore/execution-contract.json"
script_path="$(jq -er '.script_path' "${contract}")"

fail() {
  echo "crash-restore execution contract drift: $*" >&2
  exit 1
}
...
echo "crash-restore execution contract matches ${workflow_path}"
