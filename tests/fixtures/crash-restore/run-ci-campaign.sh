#!/usr/bin/env bash
# Runs the M5 crash and restore campaign the way CI runs it.
#
# The dedicated workflow provisions a runner, a database, and matching
# PostgreSQL client tools, and then calls this; how the campaign is actually
# executed lives here rather than in the workflow. The workflow's
# contract-check step verifies the important provisioning and artifact values
...
url="postgres://postgres:postgres@127.0.0.1:5432/oxide_batch_crash_restore"
...
exec cargo run --package oxide-batch-xtask -- crash-restore
