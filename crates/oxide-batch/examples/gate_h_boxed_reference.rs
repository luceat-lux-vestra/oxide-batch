//! Gate H (#153 §6) binary-size/compile-time reference: the `Boxed*`
...
//! [`gate_h_typed_reference`](../gate_h_typed_reference.rs) in isolation.
//!
//! Identical to `gate_h_typed_reference.rs` in every respect except that the
//! reader/processor/writer are wrapped in `BoxedReader`/`BoxedProcessor`/
//! `BoxedWriter` before being handed to `ChunkStep::new` -- the only
//! variable this pair of binaries isolates.
...
use oxide_batch::{
    ...
    ChunkCounts, ChunkExecutionOutcome, ChunkExecutionReport, ChunkFaultProgress, ChunkSize,
    ChunkStep, ChunkTransaction, ChunkTransactionError, ChunkTransactionManager,
    ComponentStreamIdentity, ExecutionAttempt, ExecutionContext, ExecutionCorrelation,
    JobExecutionId, JobInstanceId, JobName, StateLimits, StepExecutionId, StepName, StopSource,
};
...
fn correlation() -> ExecutionCorrelation {
    ...
    ExecutionCorrelation::new(
        JobName::new("gate_h_boxed_reference").expect("static job name is valid"),
        JobInstanceId::new(1).expect("static instance id is nonzero"),
        JobExecutionId::new(1).expect("static execution id is nonzero"),
        attempt(1),
        StepName::new("gate_h_boxed_reference").expect("static step name is valid"),
...
async fn main() -> Result<(), Box<dyn Error>> {
    let input = std::env::temp_dir().join("oxide-batch-gate-h-boxed-reference.csv");
    let output = std::env::temp_dir().join("oxide-batch-gate-h-boxed-reference-out.csv");
    ...
    let (reader, _reader_stream, _reader_contract) = delimited_file_reader::<DelimitedRecord>(
        ...
        ComponentStreamIdentity::new("gate-h.boxed-reference.reader")?,
    )?;
    let (writer, _writer_stream, _writer_contract) =
        oxide_batch::item_components::delimited_writer(
            &output,
            DelimitedDialect::csv(),
            ComponentStreamIdentity::new("gate-h.boxed-reference.writer")?,
        )?;

    let mut step = ChunkStep::new(
        StepName::new("gate-h-boxed-reference")?,
        ChunkSize::new(1_000)?,
    ...
    println!("gate-h boxed reference pipeline completed: {report:?}");
    Ok(())
}
