...
// `.github/workflows/m5-cancellation.yml` and `run-ci-campaign.sh` — by
// exact git blob identity, not by the literal presence checks alone. These
// tests drive the real script against an isolated sandbox copy of those
// files, so a mutation proves the checker's actual behaviour rather than
// one helper's return value, and never touches the repository working tree.
// ---------------------------------------------------------------------

#[test]
fn contract_check_passes_on_the_canonical_workflow_and_script() -> Result<(), Box<dyn Error>> {
    assert!(run_cancellation_contract_check(|_sandbox| Ok(()))?);
    Ok(())
}

#[test]
fn contract_check_fails_on_an_added_trigger() -> Result<(), Box<dyn Error>> {
    let passed = run_cancellation_contract_check(|sandbox| {
        insert_after(
            &sandbox.join(".github/workflows/m5-cancellation.yml"),
            "  workflow_dispatch:\n",
            "  schedule:\n    - cron: '0 0 * * *'\n",
        )
    })?;
...
fn contract_check_fails_on_an_appended_script_command() -> Result<(), Box<dyn Error>> {
    let passed = run_cancellation_contract_check(|sandbox| {
        append_line(
            &sandbox.join("tests/fixtures/cancellation/run-ci-campaign.sh"),
            "echo \"extra command\"",
        )
    })?;
    assert!(
...
/// (`tests/fixtures/cancellation/execution-contract.json` beside its working
/// directory), so the check runs exactly as CI runs it, and `mutate` can
/// never affect the real repository files.
fn run_cancellation_contract_check(
    mutate: impl FnOnce(&Path) -> Result<(), Box<dyn Error>>,
) -> Result<bool, Box<dyn Error>> {
    let root = workspace_root();
    let sandbox = Sandbox::new("cancellation-contract-check")?;

    let workflow_dir = sandbox.path().join(".github/workflows");
    fs::create_dir_all(&workflow_dir)?;
    fs::copy(
        root.join(".github/workflows/m5-cancellation.yml"),
        workflow_dir.join("m5-cancellation.yml"),
    )?;

    let fixture_dir = sandbox.path().join("tests/fixtures/cancellation");
    ...
    for name in [
        ...
        fs::copy(
            root.join("tests/fixtures/cancellation").join(name),
    ...
    let status = Command::new(&checker)
        .arg(".github/workflows/m5-cancellation.yml")
