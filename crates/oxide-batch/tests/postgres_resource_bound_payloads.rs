...
use oxide_batch::{
    ...
    MAX_SCOPED_COMPONENTS, MAX_SCOPED_DEPENDENCY_DEPTH, MAX_SELECTOR_PATH_BYTES,
    MAX_SELECTOR_PATH_SEGMENTS, MAX_TRANSITIONS, MissingParameterPolicy, NestedJobNode,
    NestedJobParameterMapping, NestedJobParameterSource, NodeId, OperationId, ParameterCoercion,
    ParameterName, ParameterRole, ParameterValue, ParameterValueKind, PartitionBudget,
    PartitionCount, PartitionKey, PartitionPlanEntry, PartitionPlanFactory,
    ...
    ScopeFactoryKind, ScopeFrameworkSource, ScopeKind, ScopeResolverKind, ScopedCleanupError,
    ScopedComponentDefinition, ScopedComponentFactory, ScopedComponentHandle, ScopedComponentId,
    ScopedComponentRegistration, ScopedFactoryContext, ScopedFactoryError, SelectorPath,
...
fn scoped_dependency_job(count: usize) -> Result<FlowJob, Box<dyn Error>> {
    ...
    let mut graph = FlowGraph::new(node.clone())
        .with_node(FlowNode::step(StepNode::new(
            node.clone(),
            step_name.clone(),
            StepComponents::Tasklet(ComponentRevision::new("scoped-dependency-tasklet-v1")?),
        )))
    ...
    let mut job = FlowJob::new(name, plan)?
        .with_tasklet_step(node, TaskletStep::new(step_name, Arc::new(CompleteTasklet)))?;
