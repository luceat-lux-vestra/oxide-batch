...
use crate::{
    ...
    JobParameters, JobRepository, LifecycleEvent, LifecycleEventKind, LifecycleEventSink,
    LifecycleTransition, ListenerContext, ListenerFailure, ListenerFailureKind, ListenerPhase,
    NodeId, RepositoryError, StepComponents, StepExecution, StepExecutionId, StepExecutionListener,
...
impl<'a> JobLauncher<'a> {
    ...
    pub(crate) async fn launch_precreated(
        ...
        self.ensure_accepting()?;
        let graph = self
            .prepare_precreated_execution_graph(job, parameters, instance_id, execution_id)
            .await?;
        self.launch_created(job, parameters, stop, graph).await
    }
    ...
    async fn prepare_precreated_execution_graph(
        &self,
        job: &TaskletJob,
        parameters: &JobParameters,
        instance_id: JobInstanceId,
        ...
        let step_execution = match existing_steps.as_slice() {
            [] => {
                unit.create_step_execution(execution_id, job.step.name())
                    .await?
            }
            [step]
