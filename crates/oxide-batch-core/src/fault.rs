//! Runtime-neutral fault-policy values.
//!
//! Retry, skip, rollback, and backoff policy are restart-relevant
//! definition data: they are fingerprint inputs and are embedded in a
//! compiled step. The cancellable waiting contract that consumes a delay
//! is not, and stays with the runtime.
...
/// The longest delay a backoff policy may produce.
const MAX_BACKOFF: Duration = Duration::from_hours(24);
/// The largest retry attempt count one step may declare.
const MAX_RETRY: u32 = 65_535;
/// The smallest bounded retry-state size one step may declare.
const MIN_RETRY_STATE: u32 = 1;
/// The largest bounded retry-state size one step may declare.
