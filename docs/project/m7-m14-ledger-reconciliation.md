...
| M7 | job/step scope, component factories, typed parameter/context late binding, scoped cleanup, optional expression resolution | `SCOPE-JOB-001`, `SCOPE-STEP-001`; `TEST-SCOPE-001` supplies an existing test-kit base only | [#196](https://github.com/luceat-lux-vestra/oxide-batch/issues/196), with conformance/exit evidence in [#200](https://github.com/luceat-lux-vestra/oxide-batch/issues/200) |
...
| M8 | separated repository/explorer/operator ports, capability negotiation/certification, dialect/migration SPI, schema/locking/ID portability, cursor/paging/keyset/streaming, batch/upsert/stored procedure, same-resource enlistment | `REPO-COMMAND-001`, `REPO-EXPLORE-001`, `REPO-OPERATOR-001`, `IO-DB-001`, `DB-POSTGRES-001`, `DB-MYSQL-001`, `DB-SQLITE-001`, `DB-SQLSERVER-001`, `DB-ENTERPRISE-001`, `DB-MONGO-001`, `META-UPGRADE-001`, `META-CONTEXT-001`, `TEST-REPO-001` | M8 tracker [#201](https://github.com/luceat-lux-vestra/oxide-batch/issues/201); existing `Verified` rows are bases, not evidence that new adapters already satisfy the M8 certification matrix |
| M8 | durable effect primitives: outbox, inbox/dedup, effect journal, idempotency, unknown-commit recovery and cross-resource delivery descriptors | No distinct Spring row is required: these are native delivery primitives behind capability-specific adapters. Their Spring-facing consequences remain represented by repository/integration rows. | Native-extension workstream owned by [#201](https://github.com/luceat-lux-vestra/oxide-batch/issues/201); M9 adapters consume the same model through #202 rather than inventing a second delivery model |
| M8 | metadata archive, purge, export, retention, backup/restore and adapter-portable lifecycle evidence | `REPO-RETENTION-001`, `META-RETENTION-001`; `META-UPGRADE-001` supplies the released schema-evolution base | [#201](https://github.com/luceat-lux-vestra/oxide-batch/issues/201); project-wide compatibility carry-forward is rechecked by M14 #208 |
| M9 | broker/stream adapters, bounded envelope, offset/checkpoint, ack/redelivery, DLQ/poison, replay/backpressure, broker-transaction/delivery profiles, schema/version metadata | `MSG-KAFKA-001`, `MSG-AMQP-001`, `MSG-OTHER-001`; M8 durable-effect primitives remain the shared outbox/inbox/idempotency base rather than new Spring rows | M9 tracker [#202](https://github.com/luceat-lux-vestra/oxide-batch/issues/202) |
...
| M10 | multi-threaded item/step processing, local chunking, dynamic local partition scaling, split scaling, ordered commit/aggregation | `SCALE-PARSTEP-001`, `SCALE-MTSTEP-001`, `SCALE-LOCALCHUNK-001`, `SCALE-LOCALPART-001`; M7 semantics for `FLOW-SPLIT-001` remain a prerequisite rather than a competing scheduler | M10 tracker [#203](https://github.com/luceat-lux-vestra/oxide-batch/issues/203) |
| M10 | declarative CPU/memory/connection/queue/broker-credit/blocking-thread/deadline budgets, bounded prefetch, graceful drain, spill policy, deterministic fallback, optional adaptive controls | No separate Spring row for the native resource-control mechanisms. Observable Spring-equivalent concurrency behavior remains in the `SCALE-*` rows. | Native execution/resource workstream [#203](https://github.com/luceat-lux-vestra/oxide-batch/issues/203); changes require workload/benchmark evidence and correctness equivalence |
...
| M10 | deterministic scheduling/commit traces and concurrency telemetry sufficient to explain scaling/resource behavior | released base in `OBS-METRICS-001`; deterministic execution semantics remain prerequisites rather than performance evidence by themselves | [#203](https://github.com/luceat-lux-vestra/oxide-batch/issues/203); later instrumentation does not retroactively broaden the existing `Verified` claim |
...
| M11 | worker registration/capabilities, durable assignment, lease/fencing/heartbeat, retry/redelivery/reassignment, protocol/version negotiation, coordinator HA, artifact trust/distribution, cancellation/drain, and approved Kafka/NATS/AMQP/direct-gRPC worker transport profiles | No additional Spring row is needed for the native protocol/HA/transport mechanism itself; the observable remote execution capability remains in the `SCALE-REMOTE*` rows, while item-integration rows remain distinct. | [#204](https://github.com/luceat-lux-vestra/oxide-batch/issues/204); protocol and transport-neutral correctness decisions are owned by [#205](https://github.com/luceat-lux-vestra/oxide-batch/issues/205). RFC-0009 is still proposed, so this mapping is not implementation authorization. |
| M12 | Java reference harness, normalized differential comparison, compatibility profiles/difference ledger, and terminal Spring parity closure | Every Spring ledger row; M12 must leave zero `Unknown`, `Deferred`, `Planned`, `Implemented`, `Partial`, or untested rows | M12 tracker [#206](https://github.com/luceat-lux-vestra/oxide-batch/issues/206) |
| M12 | neutral definition IR/extractor, mapping/difference reports, Rust stubs, one-way parameter/status/exit/context/metadata import/export with dry-run/reconciliation | `MIG-DEFINITION-001`, `MIG-METADATA-001` | [#206](https://github.com/luceat-lux-vestra/oxide-batch/issues/206), under RFC-0010 and the accepted migration contract |
...
| M13 | stable extension SDK/certification kit, first-party/certified-third-party support tiers, production reference workloads/cookbooks, deterministic replay/trace, residual effect-journal/savepoint/fork-lineage ecosystem surfaces | No new Spring row merely for the SDK/certification/support/reference-workload mechanism. Existing Spring-facing behavior stays with its semantic row; native extension/ecosystem surfaces are explicit roadmap extensions. | [#207](https://github.com/luceat-lux-vestra/oxide-batch/issues/207) |
...
| M14 | stable API/schema/manifest/configuration/protocol/support boundaries, deprecation policy, N/N-1/N-2 compatibility, certification matrix, upgrade/rollback, maintained reference workloads, project-wide RC/GA evidence, and documented control-plane extraction decision | No release-governance row is created merely for GA. Existing rows such as `META-UPGRADE-001` and `META-RETENTION-001` carry their feature semantics; every advertised capability must independently have a terminal/released disposition. | M14 tracker [#208](https://github.com/luceat-lux-vestra/oxide-batch/issues/208) |
...
4. **Outbox/inbox/effect-journal mechanism** — M8 #201 native delivery
   ...
   delivery semantics.
5. **Worker protocol, lease/fencing, coordinator HA, artifact trust and worker
   transport-profile mechanics** — M11 #204 native distributed correctness
   mechanisms, hard-gated by #205 and RFC-0009.
6. **Extension SDK/certification/support-tier/reference-workload machinery and
   deterministic trace/replay format** — M13 #207 native ecosystem mechanisms.
7. **Out-of-process/WASI isolation** — M13 #207 native extension/security
   ...
   as `STEP-CUSTOM-001`.
8. **Project-wide GA/deprecation/control-plane extraction mechanics** — M14
   #208 release/support governance, not a feature row.
