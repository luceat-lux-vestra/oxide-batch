...
| Bounded local scale | The [local-scale contract](../architecture/local-scale.md) fixes the split and partitioned-step subset, assignment identity, single-invocation partitioning, durable restart state, deterministic aggregation, structured ownership and cancellation, thread-safety validation, finite budgets, sequential-fallback equivalence, and manifest format 3. |
