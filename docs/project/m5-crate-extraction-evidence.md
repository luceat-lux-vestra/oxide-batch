...
The recommended resolution is a documentation correction to the extraction
contract that states, for stage 2, which durable record types move into
`oxide-batch-repository` and which move into `oxide-batch-core`, with `NodeId`
the leading candidate for core because it is a durable identity already
generated by core's `definition_token` macro.

Until that correction is accepted, stages 2 and 3 stay unstarted. Stage 1
stands on its own: it is complete, evidenced, and revertible, and the contract
requires a stage to begin only after the previous stage's evidence passes.
...
- M5's definition of done constrains "each completed crate extraction", so an
  incomplete stage set does not by itself block the milestone. Whether M5 exits
  with one of three stages delivered is a milestone decision, not an
  implementation one.
