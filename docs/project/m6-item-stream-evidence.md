...
| Pre-`open` schema/codec/restartability enforcement | `StreamStateContract` (`crates/oxide-batch-core/src/component_state.rs`) binds a registered stream's expected schema/codec identity, version, and restartability declaration, independent of the opaque `ItemStream` implementation. `ComponentStateEnvelope::validated_for_open` (called through the contract from `chunk_runtime`'s `inherited_progress`) validates and migrates inline state to the contract's current schema/codec versions *before* `ItemStream::open` is ever called. External state is accepted only when both recorded versions are current; older external schema/codec versions fail closed until a resolution/migration capability exists. Unknown/newer versions or any migration failure close previously-opened streams and fail without entering the application's `open`. |
