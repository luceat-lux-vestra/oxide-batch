...
| Atomic business and progress commit | One adapter-owned connection encloses parameter-bound business writes and a step CAS update covering checkpoint, execution context, read/process/write/filter/commit counters, injected-clock update time, and optimistic version. `committed_chunk_advances_checkpoint` commits two chunks and observes matching business rows, cumulative counters, and the advancing checkpoint; `writer_failure_rolls_back_business_and_checkpoint` proves the corresponding rollback. |
