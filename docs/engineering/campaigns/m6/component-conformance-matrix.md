...
The following normalized cross-check makes the remaining exit dimensions
explicit for every catalog surface. The family tables below retain the more
useful function-level references; this table prevents a short family-table
column from being mistaken for an unreviewed omission. `Inherited — ...` is a
positive contract disposition, while `N/A — ...` is a justified non-applicable
case. The retained campaign reports are the executable denominator for these
references: `m6-conformance-campaign-postgres-15.json` and
`m6-conformance-campaign-postgres-18.json`.

| Catalog surface | Malformed / partial | Failure / rollback | Stop / cancellation | Panic | Close / lifecycle | State / corruption / restart | Bounds | Diagnostics / redaction |
|---|---|---|---|---|---|---|---|---|
| `ItemReader` / `ItemProcessor` / `ItemWriter` traits | N/A — traits define typed result contracts; concrete implementations own input validation | N/A — traits do not implement a write boundary | Inherited — each implementation must observe its context token | Inherited — runtime classifies component panics | Inherited — stream/runtime owns lifecycle | Inherited — component stream contract owns durable state | Inherited — implementation contract | Inherited — typed errors and component diagnostics |
| `BoxedReader` / `BoxedProcessor` / `BoxedWriter` | Inherited — delegate owns malformed behavior | Inherited — delegate and Gate B atomicity | Inherited — delegate/context behavior | Inherited — same delegate panic classification | Inherited — same lifecycle calls | Covered — Gate B B-01..B-08; representation is not durable identity | Inherited — erasure adds no queue/buffer | Inherited — no raw item/state disclosure |
| `ItemStream` / `BoxedStream` | Covered — schema/codec mismatch and malformed envelope tests | Covered — open/update/close failure tests and rollback | Covered — stream stop and terminal cleanup tests | Covered — runtime panic classification around lifecycle | Covered — `item_stream.rs` ordering/cleanup cases | Covered — checksum, migration, corruption, version rejection, restart | Covered — state limits and external reference bounds | Covered — sensitive state is redacted |
| `IterReader`, `IdentityProcessor`, `NoopWriter` | N/A — fixture components accept their typed input by contract | N/A — no durable write or internal buffer | Covered — `identity_processor_observes_stop` | N/A — no component panic path is introduced | N/A — no owned stream lifecycle | N/A — ephemeral, stateless components | N/A — no internal unbounded storage | Covered — typed errors only |
| `FilterProcessor`, `PeekReader`, `AggregatingReader` | Covered — delegate/peek/aggregate malformed and partial cases | Covered — delegate failure, preserved lookahead/group, rollback through runtime | Covered — peek/aggregate stop tests | Inherited — wrapper cannot change delegate panic class | Inherited — delegate stream lifecycle | Inherited — restartability is exactly the delegate's; aggregate buffer is not committed progress | Covered — aggregate bound and one-item peek bound | Inherited — delegate diagnostic policy |
| `ClassifyingProcessor` / `ClassifyingWriter` / `ValidatingProcessor` | Covered — missing key and validation rejection | Covered — delegate failure propagation and enlisted write ordering | Covered — writer stop short-circuits classification | Inherited — delegate/validator panic classification | N/A — no owned stream lifecycle | N/A — stateless routing/validation wrappers | Covered — bounded delegate registration | Inherited — typed failure, no item payload logging |
| `CompositeReader`, `ChainProcessor`, `FanOutWriter`, synchronized wrappers | Inherited — delegates own input validation | Covered — composition delegate failures; rollback is runtime-owned | Inherited — delegate/context behavior | Inherited — delegate panic classification | N/A — pure wrappers do not own streams | N/A — no durable wrapper state; meet of delegate capabilities | Covered — bounded composition and synchronization | Inherited — delegate redaction |
| `Delimited*` / `FixedWidth*` | Covered — malformed records, widths, UTF-8, partial records | Covered — fault-policy and writer rollback tests | Inherited — runtime stop before commit; writer has no hidden durable shortcut | Covered — serializer/reader panic classification where applicable | Covered — close behavior through item-stream tests | Covered — PostgreSQL-backed truncation/resume and byte checkpoints | Covered — record/buffer limits | Covered — typed malformed errors and redacted diagnostics |
| `JsonArray*` / `JsonLines*` | Covered — malformed JSON, trailing garbage, CRLF, record limits | Covered — fault and rollback tests | Inherited — runtime cancellation contract | Covered — serializer failure classification | Covered — close/flush through restart harness | Covered — PostgreSQL-backed restart, truncation, short-file rejection | Covered — maximum record/payload bounds | Covered — sensitive payloads are not emitted |
| `MultiResourceReader` / `MultiResourceWriter` | Inherited — resource opener/serializer owns format validation | Covered — rollover and writer failure/rollback tests | Covered — boundary stop/retry cases | Covered — serializer panic retry evidence | Covered — resource close/rollover lifecycle | Covered — PostgreSQL-backed resource-boundary crash/restart | Covered — resource set and whole-object buffering bounds | Covered — resource metadata redaction contract |
| `InMemoryObjectStore` and object-store bridge | Covered — typed capability validation | Covered — version-token and conditional-operation failures | N/A — no asynchronous worker owns cancellation | N/A — provider-neutral in-memory fixture | Covered — object capability lifecycle | N/A — explicitly non-durable fixture; provider restart is M9 | Covered — page/continuation and object-size bounds | Covered — metadata-sensitive diagnostics are redacted |
| `PostgresCursorReader` / `PostgresPagingReader` | Covered — query/configuration and row mapping failures | Covered — cursor fault retry and enlisted transaction rollback | Inherited — runtime stop/cancellation | Covered — runtime panic classification | Covered — cursor close/recreate lifecycle | Covered — real PostgreSQL cursor loss and keyset restart | Covered — page/fetch limits | Covered — SQL values and credentials are not logged |
| `PostgresBatchWriter` | Covered — row/parameter mapping failures | Covered — enlisted transaction and Gate B rollback/atomicity | Inherited — runtime stop before commit | Covered — writer failure classification | Covered — transaction and stream close | Covered — Gate B and PostgreSQL crash/restart evidence | Covered — batch/chunk limits | Covered — database values/credentials are redacted |
| Completion policies | N/A — policies consume typed observations, not serialized input | Inherited — policy state participates in chunk rollback | Covered — stop-aware policy/runtime tests | N/A — no user component call in the policy decision | N/A — policies do not own streams | Covered — adaptive policy persistence/restart; stateless policies explicitly N/A | Covered — chunk/time thresholds | Covered — no item payload diagnostics |
| `ChunkPipelineBuilder`, `ChunkJob`, `ChunkStep`, `FlowJob` | Inherited — component and plan validators | Covered — runtime fault, rollback, transaction and Gate B cases | Covered — stop/cancellation campaign and runtime tests | Covered — panic classification and retry/terminal tests | Covered — ordered stream open/close and flow lifecycle | Covered — process-kill, multi-chunk and PostgreSQL restart | Covered — chunks, queues, workers, and retries | Covered — structured redacted diagnostics |
| Item listeners | Inherited — listener receives classified component outcome | Covered — listener failure/rollback interaction tests | Covered — stop callback behavior | Covered — listener panic classification | Covered — listener lifecycle ordering | N/A — listener registration state is not durable component state | Covered — bounded listener set | Covered — listener payload diagnostics follow redaction policy |
| `oxide-batch-test` public test-kit | Covered — fixtures expose malformed/failure injectors | Covered — failure injection and rollback assertions | Covered — deterministic stop fixtures | Covered — panic fixture and classification | Covered — fixture cleanup and lifecycle assertions | Covered — restart harness and PostgreSQL fixture tests; Gate G gaps remain explicitly documented | Covered — fixture bounds and isolated resources | Covered — test diagnostics avoid raw sensitive state |
