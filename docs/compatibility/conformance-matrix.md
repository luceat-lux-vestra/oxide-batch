...
| OPS-CLI-001 | 6.0.4 [job] | Command-line operation | Operators launch and manage jobs from a command line with stable exit codes | Minimal guarded `oxide-batch` CLI over portable services | Native equivalent | Planned | M4/M7 | No Spring context bootstrap; closed command grammar and own exit categories | R/R/R/J/R/N | — | [M4 operator CLI and configuration](../operations/operator-cli.md) | Grammar, configuration precedence, output schema, exit categories, confirmation, and secret handling are closed by the [M4 design gate](../project/m4-design-gate-evidence.md); implementation is issue #77 |
