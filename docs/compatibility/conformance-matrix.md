...
| SCALE-LOCALPART-001 | 6.0.4 [scale] | Local partitioning | Durable partitions execute in bounded local workers | Local assignment/aggregation | Behavioral | Partial | M4/M10 | No lease or fencing in the local slice | R/R/R/R/R/R | [M4 local-scale plan](../project/m4-local-scale-plan-evidence.md) | Format-3 worker/partitioner/aggregation identity, partition and worker bounds, pool-capacity validation, and canonical fingerprints are implemented; durable planning, worker execution, compare-and-swap results, restart reuse, and aggregation remain issue #80, while RFC-0009 remote semantics remain proposed |
