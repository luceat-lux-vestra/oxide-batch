...
Path('crates/oxide-batch/tests/nested_job_runtime.rs').write_text(r'''//! In-memory end-to-end evidence for M7 #265 nested-job runtime semantics.
...
#[tokio::test(flavor = "current_thread")]
async fn missing_required_mapping_fails_before_link_or_child_work()
-> Result<(), Box<dyn Error>> {
    let child_calls = Arc::new(AtomicUsize::new(0));
    let after_calls = Arc::new(AtomicUsize::new(0));
